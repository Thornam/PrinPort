...
def estimated_prediction_matrix(returns, signals, D=120, eff=False):
    ...
    Parameters:
        ...
        eff (True/False): Efficiency parameter. If True the prediction matrix is created in a more efficient way, however there can be slight differences from the more robust calculation of the prediction matrix when this is False. The prediction matrix can take up a lot of memory and take a while when number of assets (+2000) and periods becomes large.
    ...
    if eff == True:
        ...
        prediction_matrix = np.einsum('li,lj-> lij', r, s).mean(0)
...
def principal_alpha_portfolio(returns, signals, K=-1, p=2, D=120, eff=False):
    ...
    # Uses the pre-defined Prediction Matrix function
    prediction_matrix = estimated_prediction_matrix(returns, signals, D=D, eff=False)
    ...
    L_k = np.einsum('il,jl-> lij', V_sort, V_flip) - np.einsum('il,jl-> lij', V_flip, V_sort)
